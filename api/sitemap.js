...
// Fetches published articles from Supabase and generates XML
module.exports = async (req, res) => {
  ...
  try {
    const response = await fetch(`${SUPABASE_URL}/rest/v1/articles?published=eq.true&select=slug,updated_at`, {
      headers: {
    ...
    const staticPages = [
      ...
      { loc: `${SITE}/about.html`, priority: '0.3', changefreq: 'monthly' },
    ...
    for (const a of articles) {
      ...
      xml += `  <url>\n    <loc>${SITE}/article?slug=${encodeURIComponent(a.slug)}</loc>\n    <lastmod>${mod}</lastmod>\n    <changefreq>monthly</changefreq>\n    <priority>0.6</priority>\n  </url>\n`;
    }
